...
class request {
    ...
    async getAllStudent(req) {
        ...
        return new Promise(async function (resolve, reject) {
            try {
                ...
                var faculty_id = req.faculty_id
                var command;
                if (faculty_id != 0) command = `SELECT * FROM STUDENT_PROFILE WHERE student_status = 'Y' AND faculty_id = '${faculty_id}' ;`// sql command
                else command = `SELECT * FROM STUDENT_PROFILE WHERE student_status = 'Y';`// sql command
                var result = await request.query(command); //ยิง command เข้าไปใน DB
                let message = {
                    statusCode: 200,
                    message: result.recordset
    ...
    async delete(req) {
        let functionName = '[delete]' //ชื่อ function
        return new Promise(async function (resolve, reject) {
            try {
                var request = new sql.Request();
                var current_date = moment().format()
